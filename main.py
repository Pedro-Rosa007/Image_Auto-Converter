...
class Backend:
    ...
    def __init__(self):
        self.pasta_saida_personalizada = None

    def escolher_pasta_saida(self, parent=None):
        """Abre diálogo para escolher a pasta de saída das fotos"""
        pasta = filedialog.askdirectory(
            title="Escolha a pasta de saída para as fotos convertidas",
            parent=parent
        )
        if pasta:
            self.pasta_saida_personalizada = pasta
            self._mostrar_mensagem(
                "info",
                "Sucesso",
                f"Pasta de saída definida para:\n{pasta}",
                parent=parent
            )
            return pasta
        return None
    ...
    def obter_pasta_saida(self):
        """Retorna a pasta de saída: personalizada ou padrão"""
        return self.pasta_saida_personalizada if self.pasta_saida_personalizada else self.SAIDA_DIR
    ...
    def _garantir_pastas(self):
        ...
        pasta_saida = self.obter_pasta_saida()
    ...
    def converter_e_limpar(self, parent=None):
        ...
        self._mostrar_mensagem(
            "info",
            "Sucesso",
            f"Processo concluido! {convertidas} arquivo(s) convertido(s) para {pasta_saida}.",
            parent=parent,
        )
        return
